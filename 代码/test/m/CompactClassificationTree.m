...
%   Copyright 2010-2017 The MathWorks, Inc.
    ...
    methods(Hidden)
        function s = toStruct(this)
            ...
            if ischar(s.ClassSummary.ClassNames) && s.ClassSummary.ClassNamesType == 1
            ...
            % Surrogate splits not supported
            ...
            % Convert ClassNames to char
            classnames = impl.ClassNames;
            if     isempty(classnames)
                classnames = [];
                classnamesType = '';
                classnamesLength = 0;
            elseif isnumeric(classnames) || islogical(classnames) || ischar(classnames)
            ...
            % Remove dependent properties
            impl = rmfield(impl,{'CatSplit' 'CutType'});
            
            % Remove curvature and interaction
            impl = rmfield(impl,{'Curvature' 'Interaction'});
            
            % Save the modified implementation
    ...
    methods(Static,Hidden)
        function obj = fromStruct(s)
            % Make a tree object from a codegen struct.
        ...
        function obj = make(s)
            % Make a tree object from a struct (not codegen).
    ...
    methods
        ...
        %   NODE and class numbers CNUM for classification tree TREE and predictors
        %   X. X must be a table if TREE was originally trained on a table, or a
        %   numeric matrix if TREE was originally trained on a matrix. If X is a
        %   table, it must contain all the predictors used for training this model.
        %   If X is a matrix, it must have P columns, where P is the number of
        %   predictors used for training. Classification labels LABEL have the same
        %   type as Y used for training. Posterior class probabilities POSTERIOR
        %   are an N-by-K numeric matrix for N observations and K classes, where N
        %   is the number of rows in X. NODE is a numeric vector of length N. CNUM
        %   is a numeric vector of length N with classes coded as integer numbers
        %   given by the order of classes in the ClassNames property.
        %
        ...
        %   using predictors X and true class labels Y. X must be a table if TREE
        %   was originally trained on a table, or a numeric matrix if TREE was
        ...
        %   training. Y must be of the same type as TREE.ClassNames and have N
        ...
        %                     'exponential', 'logit', 'mincost', and 'quadratic'.
