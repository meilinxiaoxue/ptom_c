...
%   predictors X and class labels Y. X must be a table if OBJ was
%   originally trained on a table, or a numeric matrix if OBJ was
%   originally trained on a matrix. If X has N rows, pass Y as a vector
%   with N elements of one of the following types: single, double, logical,
%   categorical, or cell array of strings. Y can be omitted if X is a
%   table that includes the response variable.
%
...
if     isa(obj,'classreg.learning.classif.CompactClassificationSVM')
    ...
    if isa(obj,'ClassificationSVM') && ...
        ...
        if numel(varargin)<1 || (numel(varargin)==1 && ~istable(varargin{1}))
