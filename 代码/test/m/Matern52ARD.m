classdef Matern52ARD < classreg.learning.gputils.Kernel
%Matern52ARD A class that defines the Matern 5/2 kernel with a separate length scale per predictor.
%
%   Matern52ARD properties:
...
%   Copyright 2014-2015 The MathWorks, Inc.
    ...
    properties(Constant)
        ...
        Name = classreg.learning.modelparams.GPParams.Matern52ARD;              
    end
    ...
    properties(GetAccess=public,SetAccess=protected)
        ...
        %   function. The Matern52ARD kernel can be parameterized
        ...
        %   Does not apply to the Matern52ARD kernel.
        CustomFcn = [];          
    end
    ...
    methods(Static)        
        ...
        %       Theta(d+1) = log(sigmaF)
            ...
            this               = classreg.learning.gputils.Matern52ARD();
            this.Theta         = theta;
    ...
    methods      
        ...
        %
        %   As an example, for Matern52ARD with 2 predictors,
    ...
    methods
        ...
        %   KNM and DKNM at a higher memory cost.        
            ...
            % N-by-d, XM is M-by-d then D2 below is N-by-M-by-d for ARD. 
            % Force D2 to be >= 0 for the Matern52ARD kernel.
            ...
            % d = number of predictors or variables
                ...
                D   = sqrt(5)*sqrt(KNM);
                KNM = (sigmaF^2)*((1 + D.*(1 + D/3)).*exp(-D));                                
                
                % 3.3 Get DKNM if needed. Precompute the quantity 
                % ((1 + D).*KNM)./((1 + D + D.^2/3)) that appears in the 
                % expression for derivatives and save in D.
                if nargout < 2
                    return;
                end
                D = ((1 + D).*KNM)./((1 + D + D.^2/3));
        ...
        %   o KNM = N-by-M kernel matrix K(XN,XM | Theta).        
            ...
            % sigmaF do not become too small. Force the output of
            % calcDistance to be >= 0 for the Matern52ARD kernel.
            d        = length(theta) - 1;            
            sigmaL   = exp(theta(1:d));
            sigmaF   = exp(theta(d+1));            
            tiny     = this.Tiny;
