...
%   Copyright 2010-2017 The MathWorks, Inc.
    ...
    methods
        ...
        %   numbers NODE for regression tree TREE and predictors X. X must be a
        %   table if TREE was originally trained on a table, or a numeric matrix if
        %   TREE was originally trained on a matrix. X has size N-by-P, where P is
        %   the number of predictors used for training this model. YFIT is a vector
        %   of type double with size(X,1) elements. NODE is a numeric vector of
        %   length size(X,1).
        %
        ...
        %   computed using matrix of predictors X and observed response Y. X must
        %   be a table if TREE was originally trained on a table, or a numeric
        %   matrix if TREE was originally trained on a matrix. X has size N-by-P,
        %   where P is the number of predictors used for training this model. Y
        %   must be a vector of floating-point numbers with size(X,1) elements. Y
        %   can be omitted if X is a table that includes the response variable.
        %
        ...
        %                     in the table.
        %
        %   [L,SE,NLEAF,BESTLEVEL]=LOSS(TREE,X,Y,'SUBTREES',SUBTREES) returns MSE
