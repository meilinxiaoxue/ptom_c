function model = fitglm(X,varargin) % [X, y | TBL], modelDef, ...
%FITGLM Create generalized linear regression model by fitting to data.
%   GLM = FITGLM(TBL,MODELSPEC) fits the model specified by MODELSPEC to
%   variables in the table/dataset array TBL, and returns the generalized linear
...
%                         other distributions.
%      'Options'          Structure specifying control parameters for the
%                         iterative algorithm.  This argument can be
%                         created by a call to STATSET.  For parameter
%                         names and default values, type STATSET('fitglm').
%      'B0'               Vector containing initial values for the estimated
%                         coefficients. Default is to perform the first
%                         iteration based on initial fitted values derived
%                         from Y rather than from a set of coefficients.
%
%   Example:
