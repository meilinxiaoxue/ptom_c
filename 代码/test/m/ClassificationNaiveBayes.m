...
%   Copyright 2014-2017 The MathWorks, Inc.
    ...
    methods(Hidden)
        function this = ClassificationNaiveBayes (X,Y,W,modelParams,...
            ...
            if ~all(strcmp(this.DistributionNames,'mvmn')) && ~isfloat(this.PrivX) % no integers allowed
    ...
    methods (Access=protected)
        ...
        function distParams = fitMNDist (this)
            ...
            classreg.learning.classif.CompactClassificationNaiveBayes.checkMNData(this.PrivX);  % This is in the compact class because it needs to do the check during prediction, too.
            ...
            WCCX               = WCC'*X;                            % class x feature
            posteriorWtdCounts = 1 + f.*WCCX;                       % add-1 smoothing
            posteriorProbs     = posteriorWtdCounts./sum(posteriorWtdCounts,2);
            for c = 1:this.NumClasses
                if this.NonzeroProbClasses(c)
                    distParams(c,:) = num2cell(posteriorProbs(c,:));	% This row is a vector of Multinomial probabilities.
                else
                    distParams(c,:) = {[]};
                end
            end
