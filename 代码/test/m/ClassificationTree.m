...
%       X                             - Matrix or table of predictors used to train this tree.
...
%   Copyright 2010-2017 The MathWorks, Inc.
    ...
    methods(Static,Hidden)
        function [X,Y,W,userClassNames,nonzeroClassNames,rowsused] = ...
            ...
            % The special {'NaN' '<undefined>'} extraction is for
            % TreeBagger which converts labels to cellstr.            
        ...
        function [X,Y,W,dataSummary,classSummary,scoreTransform] = ...
            ...
            if responsestring
                % Caller may require Y to be a cellstr
                if isnumeric(Y) || islogical(Y)
                    Y = cellstr(num2str(Y(:)));
                elseif iscategorical(Y) || ischar(Y)
                    Y = cellstr(Y);
                end
            end

            % Get class names before any rows might be removed. The special
            % {'NaN' '<undefined>'} extraction is for TreeBagger which
            % converts labels to cellstr.
