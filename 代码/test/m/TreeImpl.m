...
%   Copyright 2012-2016 The MathWorks, Inc.
    ...
    methods(Static)
        function this = makeFromData(X,Y,W,useObs,doclass,catpred,splitcrit,...
            ...
            curvtest = false;
            intertest = false;
            if     strcmp(predictorsel,'curvature')
                curvtest = true;
            elseif strcmp(predictorsel,'interaction-curvature')
                curvtest = true;
                intertest = true;
            end
        ...
        function this = fromStruct(s)
            ...
            % Make an object from codegen struct
            ...
            if     isempty(s.ClassNames)
                classnames = {};
            elseif strcmp(s.ClassNamesType,'cellstr')
...
% ---- Get left and right conditions for display
function [condleft,condright] = getCondLR(iscont,vname,cut,vals,maxnum)
if nargin<5
    maxnum = Inf;   % max number of categories to show
end
if iscont
    if isempty(vals)
        % Split on continuous variable, cut is the split value
        condleft = sprintf('%s<%g',vname,cut);
        condright = sprintf('%s>=%g',vname,cut);
    else
        % Split on ordinal variable, cut is the split index
    ...
    % Split on categorical, cut is two sets of values/indices
