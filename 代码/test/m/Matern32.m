classdef Matern32 < classreg.learning.gputils.Kernel
%Matern32 A class that defines the Matern 3/2 kernel.
%
%   Matern32 properties:
...
%   Copyright 2014-2015 The MathWorks, Inc.
    ...
    properties(Constant)
        ...
        Name = classreg.learning.modelparams.GPParams.Matern32;              
    end
    ...
    properties(GetAccess=public,SetAccess=protected)
        ...
        %   function. The Matern32 kernel can be parameterized in terms of
        %   two positive quantities sigmaL and sigmaF:
        %
        %   sigmaL  = Length scale
        ...
        %   Does not apply to the Matern32 kernel.
        CustomFcn = [];                
    end
    ...
    methods(Access=protected)
        function this = Matern32()            
        end        
    end
    methods(Static)        
        function this = makeFromTheta(theta)
        %makeFromTheta - Make Matern32 kernel function.
        %   this = makeFromTheta(Theta) takes a 2-by-1 vector Theta and
        %   returns a Matern32 kernel object. Theta is related to length
        %   scale sigmaL and multiplier of exp term sigmaF like this:
        %
        %       Theta(1) = log(sigmaL)
        %       Theta(2) = log(sigmaF)
        
            this               = classreg.learning.gputils.Matern32();
            this.Theta         = theta;
    ...
    methods      
        ...
        %
        %   For Matern 3/2 kernel params struct has these fields:
        %
        %       Name                 = 'Matern32'
    ...
    methods
        ...
        %
        %   usecache is a logical flag. If usecache is true the Euclidean
        %   distance matrix is cached for faster evaluation of KNM and DKNM
        %   at a higher memory cost.
        
            % 1. Precompute distance matrix if usecache is true.
            usepdist = this.UsePdist;
            if usecache
                % calcDistance returns the squared Euclidean distance. So
                % take the square root for Matern32 evaluation.
            ...
            % theta(2) = log(sigmaF)
                ...
                function DKNMr = derf(r)
                    if ( r == 1 )                        
                        DKNMr = sqrt(3)*D/sigmaL;
                        DKNMr = KNM.*((DKNMr.^2)./(1 + DKNMr));                                               
                    elseif ( r == 2 )
                        DKNMr = 2*KNM;
                    end
                end
        ...
        %   o KNM = N-by-M kernel matrix K(XN,XM | Theta).        
            ...
            % sigmaF do not become too small.
            sigmaL   = exp(theta(1));
            sigmaF   = exp(theta(2));
            tiny     = this.Tiny;
            sigmaL   = max(sigmaL,tiny);
            ...
            function KNM = f(XN,XM)                
                % 2.1 Compute sqrt(3)*D/sigmaL where D is the Euclidean distance matrix.
