...
%SVMParams Parameters for Support Vector Machine.
%
...
%       ClipAlphas               - Clip alpha coefficients to bounds or not.
%       DeltaGradientTolerance   - Tolerance on the gradient difference
%                                  between upper and lower pool violators
%                                  for SMO.
%       Epsilon                  - Half of the width of the Epsilon-insensitive band in SVM regression.
...
%                                  data.
%       RemoveDuplicates         - Remove duplicates in the training data
%                                  prior to fitting.
...
%   Copyright 2013-2017 The MathWorks, Inc.
    ...
    methods(Static,Hidden)
        ...
        function [holder,extraArgs] = make(type,varargin) 
            ...
            %nu parameter is only valid for classification
            if ~isempty(nu) 
                if ~doclass 
        ...
        function this = loadobj(obj)
            ...
            if ismember('Version',found) && ~isempty(obj.Version) ...
                ...
                if ismember('RemoveDuplicates',found) ...
                ...
                if ismember('ClipAlphas',found) && ~isempty(obj.ClipAlphas)
        ...
        function this = fillDefaultParams(this,X,Y,W,dataSummary,classSummary) %#ok<INUSL>
            ...
            if isempty(this.BoxConstraint)
                if ~doclass && strcmpi(this.KernelFunction, 'gaussian')
                   % don't set BoxConstraint here for regression with
                   % Gaussian kernel. 
                   % The BoxConstraint will be set in RegressionSVM
                else
            ...
            if ~isempty(this.Alpha) 
                ...
                else %two-class classification or regression
                    if ~isempty(this.BoxConstraint)
                        if any(abs(alphas) > this.BoxConstraint)
            ...
            elseif this.OutlierFraction>0
                if ~(doclass && numel(classSummary.NonzeroProbClasses)==1)
                    %The following sanity check needs to be inside
                    %fillDefaultParams to get the value of Solver.
...
%             end
            ...
            if isempty(this.RemoveDuplicates)
                this.RemoveDuplicates = false;
            end
    
            % if ~doclass && isempty(this.Epsilon)
            %  % do nothing here
              %  Epsilon for regression will be set in the constructor of RegressionSVM
           %   end
