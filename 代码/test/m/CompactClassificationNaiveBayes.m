...
%   Copyright 2014-2017 The MathWorks, Inc.
    ...
    methods
        ...
        %   [LABEL,POSTERIOR,COST]=PREDICT(NB,X) returns predicted class labels
        %   LABEL, posterior probabilities POSTERIOR and misclassification costs
        %   COST for model NB and predictors X. X must be a table if NB was
        %   originally trained on a table, or a numeric matrix if NB was
        ...
        %   misclassification cost.
            ...
            % If P(x|c)=0 for all c, then x provides no information about
            % c. In that case, set P(c|x)=P(c).
        ...
        %   LOGP = LOGP(NB,X) returns a column vector with N elements for N rows in
        %   the predictors X. X must be a table if NB was originally trained on
        %   a table, or a numeric matrix if NB was originally trained on a
        %   matrix. If X is a table, it must contain all the predictors used for
        %   training this model. If X is a matrix, it must have P columns, where P
        %   is the number of predictors used for training. Element I in this vector
        %   is the log of the probability of observing row I of X computed using
        %   the naive Bayes model NB. LOGP is NaN for rows of X containing any
        %   NaNs.
    ...
    methods (Access=protected)
        ...
        function [logPxc,wasnan] = score (this, X)
            ...
            % X should be converted from table to matrix before calling
            % score
            if nargout>=2
                    ...
                    if this.NonzeroProbClasses(c)
                        ...
                        for d = 1:this.NumDims
                            logPxdgc(:,d) = univariateLogP(X(:,d), this.DistributionNames{d}, ...
                                this.DistributionParameters{c,d}, catlevels{d});    % Fill in column d.
                        end
